...
class Parser:
    ...
    def p_program(self, p):
        ...
        p[0] = Program(p[1], lineno = p.lineno)
        print(p[0])
    ...
    def p_declaration_statement(self,p):
        ...
        p[0] = Declaratoin_Statement(p[2], lineno = p.lineno)
    ...
    def p_declaration(self, p):
        ...
        p[0] = Declaration(p[1], p[2], initialization = p[3], lineno = p.lineno)

    def p_initialization(self,p):
        'initialization : ASSIGN expression'
        # p[0] = ('Initialization', p[1], p[2], p.lineno)

        p[0] = Initialization(p[2], lineno = p.lineno)
    ...
    def p_identifier(self,p):
        ...
        p[0] = Identifier(p[1], lineno = p.lineno)
    ...
    def p_synonym_statement(self, p):
        ...
        p[0] = Synonym_Statement(p[2], lineno = p.lineno)
    ...
    def p_synonym_definition(self, p):
        ...
        if (len(p) == 4):
            ...
            p[0] = Synonym_Definition(identifier_list = p[1], constant_expression = p[3], lineno = p.lineno)
        elif (len(p) == 5):
            # p[0] = ('Synonym Definition', p[1], p[2], p[3], p[4], p.lineno)
            p[0] = Synonym_Definition(identifier_list = p[1], mode = p[2], constant_expression = p[4], lineno = p.lineno)

    def p_constant_expression(self, p):
        'constant_expression : expression'
        # p[0] = ('Constant Expression', p[1], p.lineno)

        p[0] = Constant_Expression(p[1], lineno = p.lineno)

    def p_newmode_statement(self, p):
        'newmode_statement : TYPE newmode_list SEMI'
        # p[0] = ('Newmode Statement', p[1], p[2], p[3], p.lineno)

        p[0] = Newmode_Statement(p[2], lineno = p.lineno)
    ...
    def p_mode_definition(self, p):
        ...
        p[0] = Mode_Definition(p[1], p[3], lineno = p.lineno)
    ...
    def p_mode_name(self, p):
        ...
        p[0] = Mode_Name(p[1], lineno = p.lineno)
...
#        p[0] = ('discrete_mode_name', p[1], p.lineno)
    ...
    def p_element_mode(self, p):
        ...
        p[0] = Element_Mode(p[1], lineno = p.lineno)

    def p_integer_expression(self, p):
        '''integer_expression : expression'''
        # p[0] = ('integer_expression', p[1], p.lineno)

        p[0] = Integer_Expression(p[1], lineno = p.lineno)
    ...
    def p_dereferenced_reference(self, p):
        ...
        p[0] = Dereferenced_Reference(p[1], lineno = p.lineno)
    ...
    def p_string_slice(self, p):
        ...
        p[0] = String_Slice(p[1], p[3], p[5], lineno = p.lineno)
...
#        p[0] = ('string_location', p[1], p.lineno)
    ...
    def p_array_element(self, p):
        ...
        p[0] = Array_Element(p[1], p[3], lineno = p.lineno)
    ...
    def p_array_slice(self, p):
        ...
        p[0] = Array_Slice(p[1], p[3], p[5], lineno = p.lineno)

    def p_array_location(self, p):
        '''array_location : location'''
        # p[0] = ('array_location', p[1], p.lineno)

        p[0] = Array_Location(p[1], lineno = p.lineno)
    ...
    def p_primitive_value(self, p):
        ...
        p[0] = ('primitive_value', p[1], p.lineno)
    ...
    def p_integer_literal(self, p):
        ...
        # p[0] = ('integer_literal', p[1], p.lineno)

        p[0] = Integer_Literal(p[1], lineno = p.lineno)

    def p_boolean_literal(self, p):
        '''boolean_literal : FALSE
                           | TRUE'''
        # p[0] = ('boolean_literal', p[1], p.lineno)

        p[0] = Boolean_Literal(p[1], lineno = p.lineno)

    #CCONST ESTA SENDO USADO CORRETAMENTE???
    def p_character_literal(self, p):
        '''character_literal : CCONST '''
        # p[0] = ('character_literal', p[1], p.lineno)

        p[0] = Character_Literal(p[1], lineno = p.lineno)

    def p_empty_literal(self, p):
        '''empty_literal : NULL'''
        # p[0] = ('empty_literal', p[1], p.lineno)

        p[0] = Empty_Literal(p[1], lineno = p.lineno)

    #SCONST ESTA SENDO USADO CORRETAMENTE???
    def p_character_string_literal(self, p):
        '''character_string_literal : SCONST'''
        # p[0] = ('character_string_literal', p[1], p.lineno)

        p[0] = Character_String_Literal(p[1], lineno = p.lineno)
    ...
    def p_value_array_element(self, p):
        ...
        p[0] = Value_Array_Element(p[1], p[3], lineno = p.lineno)
    ...
    def p_conditional_expression(self, p):
        ...
        if (len(p) == 6):
            ...
            p[0] = ('conditional_expression', p[1], p[2], p[3], p[4], p[5], p[6], p.lineno)
    ...
    def p_elsif_expression(self, p):
        '''elsif_expression : ELSIF boolean_expression then_expression
                            | elsif_expression ELSIF boolean_expression then_expression '''
        if(len(p) == 4):
            p[0] = ('elsif_expression', p[1], p[2], p[3], p.lineno)
        elif(len(p) == 5):
            p[0] = ('elsif_expression', p[1], p[2], p[3], p[4], p.lineno)
    ...
    def p_action_statement(self, p):
        ...
        if (len(p) == 3):
            p[0] = ("action_statement", p[1], p[2], p.lineno)
        elif (len(p) == 5):
            p[0] = ("action_statement", p[1], p[2], p[3], p[4], p.lineno)
    ...
    def p_if_action(self, p):
        ...
        if (len(p) == 5):
            p[0] = ("if_action", p[1], p[2], p[3], p[4], p.lineno)
        elif (len(p) == 6):
            p[0] = ("if_action", p[1], p[2], p[3], p[4], p[5], p.lineno)

    def p_then_clause(self, p):
        '''then_clause :  THEN
                       |  THEN action_statement_list'''
        if (len(p) == 2):
            p[0] = ("then_clause", p[1], p.lineno)
        elif (len(p) == 3):
            p[0] = ("then_clause", p[1], p[2], p.lineno)
    ...
    def p_else_clause(self, p):
        ...
        if (len(p) == 2):
            p[0] = ("else_clause", p[1], p.lineno)
        elif (len(p) == 3):
            p[0] = ("else_clause", p[1], p[2], p.lineno)
        elif (len(p) == 4):
            p[0] = ("else_clause", p[1], p[2], p[3], p.lineno)
        elif (len(p) == 5):
            p[0] = ("else_clause", p[1], p[2], p[3], p[4], p.lineno)
    ...
    def p_do_action(self, p):
        ...
        if (len(p) == 3):
            p[0] = ("do_action", p[1], p[2], p.lineno)
        elif (len(p) == 5):
            p[0] = ("do_action", p[1], p[2], p[3], p[4], p.lineno)
        elif (len(p) == 6):
            p[0] = ("do_action", p[1], p[2], p[3], p[4], p[5], p.lineno)
    ...
    def p_control_part(self, p):
        ...
        if (len(p) == 2):
            ...
            p[0] = ("control_part", p[1], p[2], p.lineno)
    ...
    def p_step_enumeration(self, p):
        ...
        if (len(p) == 5):
            p[0] = ("step_enumeration", p[1], p[2], p[3], p[4], p.lineno)
        elif (len(p) == 6):
            p[0] = ("step_enumeration", p[1], p[2], p[3], p[4], p[5], p.lineno)
        elif (len(p) == 7):
            p[0] = ("step_enumeration", p[1], p[2], p[3], p[4], p[5], p[6], p.lineno)
    ...
    def p_range_enumeration(self, p):
        ...
        if (len(p) == 4):
            p[0] = ("range_enumeration", p[1], p[2], p[3], p.lineno)
        elif (len(p) == 5):
            p[0] = ("range_enumeration", p[1], p[2], p[3], p[4], p.lineno)
    ...
    def p_procedure_call(self, p):
        ...
        if (len(p) == 4):
            p[0] = ("procedure_call", p[1], p[2], p[3], p.lineno)
        elif (len(p) == 5):
            p[0] = ("procedure_call", p[1], p[2], p[3], p[4], p.lineno)
...
#        p[0] = ("procedure_name", p[1], p.lineno)
    ...
    def p_exit_action(self, p):
        ...
        p[0] = ("exit_action", p[1], p[2], p.lineno)

    def p_return_action(self, p):
        '''return_action :  RETURN
                         |  RETURN result'''
        if (len(p) == 2):
            p[0] = ("return_action", p[1], p.lineno)
        elif (len(p) == 3):
            p[0] = ("return_action", p[1], p[2], p.lineno)
    ...
    def p_builtin_call(self, p):
        ...
        if (len(p) == 4):
            p[0] = ("builtin_call", p[1], p[2], p[3], p.lineno)
        elif (len(p) == 5):
            p[0] = ("builtin_call", p[1], p[2], p[3], p[4], p.lineno)
    ...
    def p_procedure_definition(self, p):
        ...
        if (len(p) == 3):
            p[0] = ("procedure_definition", p[1], p[2], p.lineno)
        elif (len(p) == 4):
            p[0] = ("procedure_definition", p[1], p[2], p[3], p.lineno)
    ...
    def p_formal_procedure_head(self, p):
        ...
        if (len(p) == 5):
            p[0] = ("formal_procedure_head", p[1], p[2], p[3], p[4], p.lineno)
        elif (len(p) == 6):
            p[0] = ("formal_procedure_head", p[1], p[2], p[3], p[4], p[5], p.lineno)
        elif (len(p) == 7):
            p[0] = ("formal_procedure_head", p[1], p[2], p[3], p[4], p[5], p[6], p.lineno)
    ...
    def p_parameter_spec(self, p):
        ...
        if(len(p) == 2):
            p[0] = ("parameter_spec", p[1], p.lineno)
        elif(len(p) == 3):
            p[0] = ("parameter_spec", p[1], p[2], p.lineno)
    ...
    def p_result_spec(self, p):
        ...
        if(len(p) == 5):
            p[0] = ("result_spec", p[1], p[2], p[3], p[4], p.lineno)
        elif(len(p) == 6):
            p[0] = ("result_spec", p[1], p[2], p[3], p[4], p[5], p.lineno)
